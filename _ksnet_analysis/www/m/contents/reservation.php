...
<div id="container">
	...
	<div class="real-cont">
		...
		<form id="res_form" name="res_form" method="post"  _action="reservation3.php">
				...
				<?
					...
					<li>
						<table class="info">
							...
							<tbody>
								<tr>
									<td class="tit"><a href="tour_view.php?pid=<?=$row[pid]?>"><?=stripslashes($row[wr_subject])?> <?//=stripslashes($row[wr_2])?></a></td>
									<td class="date"><?=$row[tourDay]?></td>
									<td class="opt" style="text-align:center"><?=$row['membCnt']?>명</td>
								</tr>
							</tbody>
						</table>
						<table class="price" style="margin-top:5px">
							<tbody>
								...
								<tr>
									<td>총 상품금액</td>
									<td><big><?=number_format($row[total_fee4])?></big>원</td>
								</tr>
							</tbody>
						</table>
					</li>
			...
			<div class="order-tot">
				<table>
					...
					<tbody>
						...
						<tr>
							<td class="txt">중도금</td>
							<td class="price text-sky"><?=number_format($total_fee2)?></big>원</td>
						</tr>
						<?}?>
						<? if($total_fee_air) {?>
						<tr>
							<td class="txt">항공비</td>
							<td class="price text-sky"><?=number_format($total_fee_air)?></big>원</td>
						</tr>
						<?}?>
						<tr>
							<td class="txt">잔금</td>
							<td class="price text-sky"><?=number_format($total_fee3)?></big>원</td>
						</tr>
					</tbody>
				</table>
			</div>
					...
					<li>
						...
						<table class="price">
							<tbody>
								...
								<tr>
									<td>예약금</td>
									<td><big><?=number_format($row[total_fee1])?></big>원</td>
								</tr>
								<tr>
									<td>현지지불금</td>
									<td><big><?=number_format($row[total_fee2])?></big>유로</td>
								</tr>
							</tbody>
						</table>
					</li>
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td class="price"><big><?=number_format($tt_fee2)?"<big>".number_format($tt_fee2)."</big>유로":"없음"?></td></big></td>
						</tr>
					</tbody>
				</table>
			</div>
			...
			<div class="order-form">
				...
				<table>
					...
					<tbody>
						<tr>
							<th>이름</th>
							<td>
								<input type="text" name="mb_name" value="<?=$member[mb_name]?>" class="input" style="width:100%" placeholder="한글이름" required>
							</td>
						</tr>
						<tr>
							<th>연락처</th>
							<td>
								<input type="text" name="mb_hp" value="<?=$member[mb_hp]?>" class="input" style="width:100%" placeholder="연락처" required>
							</td>
						</tr>
						<tr>
							<th>이메일</th>
							<td>
								<input type="text" name="mb_email" value="<?=$member[mb_email]?>" class="input" style="width:100%" placeholder="이메일" required>
							</td>
						</tr>
						<tr>
							<th>카카오톡 ID</th>
							<td>
								<input type="text" name="mb_kakao" class="input" style="width:100%" placeholder="카카오톡 ID" _required>
							</td>
						</tr>
				...
				if($is_delivery) {
						...
						<table>
							<tbody>
								<tr>
									<td  style="text-align:left">[사은품] <br>
									혜택 1 &nbsp; 
										<select name="gift_1" class="select" required style="width:100px" >
											<option value="" selected>선택해 주세요
											<option value="선택안함" >선택안함
											<option value="유럽여행백서" >유럽여행백서
											<option value="이탈리아여행백서" disabled>이탈리아여행백서(절판)
										</select>
										&nbsp; &nbsp; &nbsp; 혜택 2 &nbsp; 
										<select name="gift_2" class="select" required style="width:100px" >
											<option value="" selected>선택해 주세요
											<option value="선택안함" >선택안함
								...
								<tr>
									...
									<?
									if($is_iphone) { ?>
										<input type="text" name="addr1" value="" class="input" style="width:100%" placeholder="주소" required ><br>
									...
									<input type="text" name="zip" value="" class="input" style="width:100px" placeholder="우편번호" readonly required onclick="win_zip('res_form', 'zip', 'addr1', 'addr2', 'addr3', 'addr_jibeon');"><br>
									<input type="text" name="addr1" value="" class="input" style="width:100%" placeholder="주소" required onclick="win_zip('res_form', 'zip', 'addr1', 'addr2', 'addr3', 'addr_jibeon');"><br>
									<input type="text" name="addr2"  value=""class="input" style="width:100%" placeholder="상세주소" required>
									<input type="hidden" name="addr3">
									<input type="hidden" name="addr_jibeon">
									<?}?>
				...
				<table>
					...
					<tbody>
						<tr>
							<th>한글이름</th>
							<td>
								<input type="text" name="ISEC_name[]" class="input ISEC" style="width:100%" placeholder="한글이름" required>
							</td>
						</tr>
						<tr>
							<th>영문이름(여권)</th>
							<td>
								<input type="text" name="ISEC_ename_1[]" class="input ISEC" style="width:35%" placeholder="성" required>
								<input type="text" name="ISEC_ename_2[]" class="input ISEC" style="width:62%" placeholder="이름" required>
							</td>
						</tr>
						<tr>
							<th>생년월일</th>
							<td>
								<input type="text" name="ISEC_birth[]" class="input ISEC" style="width:100%" placeholder="YYYY-MM-DD" required>
							</td>
						</tr>
						<tr>
							<th>국제학생증</th>
							<td>
								<input type="text" name="ISEC_no[]" class="input ISEC" style="width:60%" placeholder="국제학생증 번호" required>
								<input type="text" name="ISEC_expired[]" class="input ISEC" style="width:38%" placeholder="유효기간" required>
							</td>
						</tr>						
					</tbody>
				</table>
			...
			<? if($is_ticket) {
				//booking_mb_list 에서 확인한 티켓 구입할 인원. 티켓이 필요한 rid를 배열로 반환
				echo get_ticket_fee($ticket_rid,"mobile");
			}?>
			...
			<? if($is_passport) {?>
					...
					<div class="order-form">
						...
						<?
							...
							?>
							<table style="<? if($i>1) echo 'border-top: 1px solid #fec900;'?>" class="">
							<colgroup>
							...
							<tbody>
								<tr>
									<td style="border-bottom: 0px solid #dddcda;"><input type="text" name="passport_name_ko[<?=$i?>]" class="input" style="width:100%" placeholder="한글이름 " required></td>
									<td style="border-bottom: 0px solid #dddcda;"><input type="text" name="passport_name_en[<?=$i?>]" class="input" style="width:100%" placeholder="영문이름" required></td>
									<td style="border-bottom: 0px solid #dddcda;"><input type="text" name="passport_birth[<?=$i?>]" class="input" style="width:100%" placeholder="YYYY-MM-DD" required></td>
								</tr>
								<tr>
									...
									<td>
										<label><input type="radio" name="passport_sex[<?=$i?>]" value="남" class="passport_sex" required> 남</label>
										<label><input type="radio" name="passport_sex[<?=$i?>]" value="여" class="passport_sex" required> 여</label>
									</td>
								</tr>
							</tbody>
						</table>
	...
	<div class="real-cont">
		...
		<form action="">
				...
				<?
					...
					<li>
						...
						if($row[isEvent]=="Y") {
							...
							<tbody>
								<tr>
									<td><span class="btn-pack large orange round" style="cursor:default">이벤트 투어</span></td>
								</tr>
							</tbody>
						</table>
							<?
			...
			<div class="order-form">
				...
				<div class="order-info-box">
					<?
						if($nation=="제주도") echo stripslashes($config['cf_res_ok_jeju_m']);
						else if($nation=="패키지") echo stripslashes($config['cf_res_ok_pkg_m']);
                        else if (stripos($nation, '세미패키지') !== false) echo stripslashes($config['cf_res_ok_pkg_pc']);
						//else if($nation=="티켓") echo stripslashes($config['cf_ticket_m']);
						else echo stripslashes($config['cf_res_ok_m']);
							?>
				</div>
			</div>
			...
			<div class="buttons">
				<div class="cen">							
				<!-- 	<input type="submit" value="결제하기" class="btn-pack large focus round"> -->
	...
	<div class="real-cont">
		...
		<input type="hidden" name="rid" value="<?=$rid?>">
			...
			<div class="regis-agree">
				<div class="mgb5">
					<h3 class="inline bold">투어 환불 규정</h3>
					<!-- <a href="guide_cancel.php" target="_blank" class="btn-pack small">전문보기</a> -->
				</div>
				<div class="box"><? include($_SERVER[DOCUMENT_ROOT] . "/contents/_cancel.php"); //include "_cancel.php"; ?></div>
				<div class="check">
					<input type="checkbox" name="agree" id="agree"> <label for="agree">투어 환불/취소 규정에 동의합니다.</label>
				</div>
			</div>
			<div class="buttons">
				<div class="cen">
