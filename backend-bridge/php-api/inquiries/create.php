...
function uno_api_inquiry_has_attachment()
{
...
function uno_api_inquiry_validate_attachment()
{
    if (!uno_api_inquiry_has_attachment()) {
        return null;
    }
...
function uno_api_inquiry_upload_dir()
{
    if (defined('G5_DATA_PATH')) {
        return G5_DATA_PATH . '/file/cusTour';
    }

    return dirname(__DIR__, 2) . '/bbs/data/file/cusTour';
}

function uno_api_inquiry_save_attachment($table, $wrId, $attachment, $now)
{
    if (!$attachment) {
        return;
    }
    ...
    $uploadDir = uno_api_inquiry_upload_dir();
    if (!is_dir($uploadDir)) {
    ...
    $storedFile = date('YmdHis') . '_inquiry_' . $wrId . '_' . bin2hex(random_bytes(4)) . '.' . $attachment['extension'];
    ...
    $source = uno_api_inquiry_escape($attachment['sourceName']);
    $stored = uno_api_inquiry_escape($storedFile);
...
function uno_api_inquiry_insert_thread($table, $member, $subject, $content, $now, $ip, $attachment)
{
    ...
    uno_api_inquiry_save_attachment($table, $wrId, $attachment, $now);
...
function uno_api_inquiry_insert_comment($table, $thread, $member, $content, $now, $ip, $attachment)
{
    $memberId = isset($member['mb_id']) ? (string) $member['mb_id'] : '';
    $memberName = isset($member['mb_name']) && $member['mb_name'] !== ''
        ? (string) $member['mb_name']
    ...
    uno_api_inquiry_save_attachment($table, $commentId, $attachment, $now);
...
$attachment = uno_api_inquiry_validate_attachment();

$result = $thread && !empty($thread['wr_id'])
    ? uno_api_inquiry_insert_comment($table, $thread, $member, $content, $now, $ip, $attachment)
    : uno_api_inquiry_insert_thread($table, $member, $subject, $content, $now, $ip, $attachment);
