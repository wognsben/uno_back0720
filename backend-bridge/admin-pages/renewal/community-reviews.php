<?php
/*
 * community-reviews.php
 * Renewal admin page for the public travel review board only.
 *
 * Scope:
 * - Uses legacy Gnuboard board `bo_table=write`.
 * - This is the public travel review community board.
 * - This is NOT public Q&A (`qna`), 1:1 inquiry (`cusTour`), or product-detail FAQ (`g5_faq`).
 */

require_once __DIR__ . '/_guard.php';
require_once __DIR__ . '/_layout.php';

uno_renewal_admin_require_access('/admin/renewal/community-reviews.php');

function uno_renewal_review_escape_db($value)
{
...
function uno_renewal_review_query($sql)
{
    return function_exists('sql_query') ? sql_query($sql) : false;
}

function uno_renewal_review_fetch($sql)
{
...
function uno_renewal_review_fetch_array($result)
{
    return $result && function_exists('sql_fetch_array') ? sql_fetch_array($result) : false;
}

function uno_renewal_review_write_table()
{
    global $g5;
...
function uno_renewal_review_text($value, $limit = 0)
{
...
function uno_renewal_review_date($value)
{
    if ($value === null || $value === '') {
        return '-';
    }
...
$table = uno_renewal_review_write_table();
$keyword = isset($_GET['keyword']) ? trim((string) $_GET['keyword']) : '';

$where = array("wr_is_comment = '0'");

if ($keyword !== '') {
    $safeKeyword = uno_renewal_review_escape_db($keyword);
...
$rows = array();
$result = uno_renewal_review_query(
    "select wr_id, wr_subject, wr_content, wr_name, mb_id, wr_datetime, wr_last, wr_comment, wr_hit, wr_good, wr_ip
       from {$table}
      where {$whereSql}
      order by wr_id desc
      limit 100"
...
while ($row = uno_renewal_review_fetch_array($result)) {
    $rows[] = $row;
}

$today = defined('G5_TIME_YMD') ? G5_TIME_YMD : date('Y-m-d');
$totalRow = uno_renewal_review_fetch("select count(*) as cnt from {$table} where wr_is_comment = '0'");
$todayRow = uno_renewal_review_fetch("select count(*) as cnt from {$table} where wr_is_comment = '0' and substr(wr_datetime, 1, 10) = '{$today}'");
$commentRow = uno_renewal_review_fetch("select sum(wr_comment) as cnt from {$table} where wr_is_comment = '0'");

uno_renewal_admin_render_head('여행후기 관리');
uno_renewal_admin_render_header();
uno_renewal_admin_render_pagehead(
    'COMMUNITY REVIEW',
    '여행후기 관리',
    '커뮤니티 여행후기 게시판입니다. 기존 우노트래블 기준 bo_table=write 데이터를 조회합니다.',
    array(
        array('label' => '기존 후기 관리자', 'href' => '/admin/board.php?bo_table=write', 'secondary' => true),
        array('label' => '프런트 후기', 'href' => '/community/review', 'secondary' => true, 'target' => '_blank'),
    )
);
?>
...
<section class="uno-admin-grid">
  ...
  <article class="uno-admin-card">
    <div>
      <h3>댓글 합계</h3>
      <p><?php echo number_format((int) ($commentRow['cnt'] ?? 0)); ?>건</p>
    </div>
  </article>
</section>

<section class="uno-admin-panel" style="margin-top:16px;">
  <form method="get" style="display:grid;grid-template-columns:minmax(220px,1fr) auto;gap:8px;align-items:center;">
  ...
  <table style="width:100%;border-collapse:collapse;min-width:1080px;table-layout:fixed;">
    ...
    <thead>
      <tr>
        ...
        <th style="text-align:left;border-bottom:1px solid var(--uno-line);padding:12px;">반응</th>
    ...
    <tbody>
      <?php if (!$rows) { ?>
        <tr>
          <td colspan="6" style="padding:28px 12px;color:var(--uno-muted);">여행후기 내역이 없습니다.</td>
        </tr>
      <?php } ?>
      <?php foreach ($rows as $row) {
          $wrId = isset($row['wr_id']) ? (int) $row['wr_id'] : 0;
          $subject = uno_renewal_review_text($row['wr_subject'] ?? '');
          if ($subject === '') {
              $subject = '(제목 없음)';
          }
          $legacyView = '/admin/board.php?bo_table=write&wr_id=' . $wrId;
          $frontView = '/community/review/' . $wrId;
      ?>
        <tr>
          ...
          <td style="border-bottom:1px solid var(--uno-line);padding:12px;">
            ...
            <div style="margin-top:6px;color:var(--uno-muted);font-size:12px;max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
              <?php echo uno_renewal_admin_escape(uno_renewal_review_text($row['wr_content'] ?? '', 140)); ?>
            </div>
          ...
          <td style="border-bottom:1px solid var(--uno-line);padding:12px;">
            <?php echo uno_renewal_admin_escape(uno_renewal_review_text($row['wr_name'] ?? '')); ?>
            <div style="color:var(--uno-muted);font-size:12px;"><?php echo uno_renewal_admin_escape((string) ($row['mb_id'] ?? '')); ?></div>
          </td>
          <td style="border-bottom:1px solid var(--uno-line);padding:12px;"><?php echo uno_renewal_admin_escape(uno_renewal_review_date($row['wr_datetime'] ?? '')); ?></td>
          <td style="border-bottom:1px solid var(--uno-line);padding:12px;">
            <span class="uno-admin-chip">댓글 <?php echo (int) ($row['wr_comment'] ?? 0); ?></span>
            <span class="uno-admin-chip" style="margin-top:6px;">추천 <?php echo (int) ($row['wr_good'] ?? 0); ?></span>
          </td>
          <td style="border-bottom:1px solid var(--uno-line);padding:12px;white-space:nowrap;">
            <a class="uno-admin-button secondary" href="<?php echo uno_renewal_admin_escape($legacyView); ?>">수정/관리</a>
